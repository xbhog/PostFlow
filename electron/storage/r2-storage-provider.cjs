...
function classifyStorageError(error) {
  ...
  if (status === 404 || name.includes('NoSuchBucket') || message.toLowerCase().includes('bucket')) {
...
function wrapStorageError(error) {
  if (error?.code && String(error.code).startsWith('R2_')) return error;
...
class R2StorageProvider {
  ...
  async testConnection() {
    ...
    let testObjectCleaned = false;
    try {
      testObjectCleaned = await this.delete(objectKey);
    } catch {
      testObjectCleaned = false;
    }

    if (!publicReachable) {
      const error = new Error(`测试对象已上传，但公开 URL 无法访问${publicStatus ? `（HTTP ${publicStatus}）` : ''}。`);
      error.code = 'PUBLIC_URL_UNREACHABLE';
      error.testObjectCleaned = testObjectCleaned;
      throw error;
    }
